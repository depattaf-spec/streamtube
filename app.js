...
function updateUser(u, patch) { const us = getUsers(); us[u] = {...us[u], ...patch}; saveUsers(us); }
...
function isFavorite(id) { return getFavorites().some(f => f.videoId === id); }
...
function playSong(videoId) {
  ...
  if (ytPlayer && ytPlayer.loadVideoById) {
    ...
    setTimeout(() => playSong(videoId), 500);
  }
}
...
async function searchYouTube() {
  ...
  try {
    ...
    if (data.error) { grid.innerHTML = '<div class="empty-state"><div class="empty-icon">⚠️</div><h3>API Error</h3><p>' + esc(data.error.message) + '</p></div>'; return; }
    const items = data.items || [];
    if (!items.length) { grid.innerHTML = '<div class="empty-state"><div class="empty-icon">🎵</div><h3>No results</h3><p>Try a different search.</p></div>'; return; }
    ...
    console.error(err);
    grid.innerHTML = '<div class="empty-state"><div class="empty-icon">⚠️</div><h3>Network error</h3><p>Could not reach the YouTube API.</p></div>';
  }
}
...
function songCard(song, opts={}) {
  ...
  const favOn = isFavorite(videoId);
  let actions = '<button class="btn-card btn-card-play" onclick="playSong('' + videoId + '')">▶ Play</button>';
  if (fav) actions += '<button class="btn-card btn-card-fav' + (favOn?' on':'') + '" id="fav-' + videoId + '" onclick="toggleFav('' + videoId + '',this)">❤</button>';
  if (removeFav) actions += '<button class="btn-card btn-card-remove" onclick="removeFav('' + videoId + '')">✕ Remove</button>';
  if (pl) actions += '<button class="btn-card btn-card-pl" onclick="openPlaylistModal('' + videoId + '')">+ List</button>';
  if (removePl && playlistId !== null) actions += '<button class="btn-card btn-card-remove" onclick="removeSongFromPlaylist(' + playlistId + ','' + videoId + '')">✕</button>';
  return '<div class="song-card"><img src="' + esc(thumbnail) + '" alt="" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22320%22 height=%22180%22%3E%3Crect width=%22320%22 height=%22180%22 fill=%22%231e1e1e%22/%3E%3Ctext x=%22160%22 y=%2298%22 fill=%22%23444%22 font-size=%2236%22 text-anchor=%22middle%22%3E%E2%99%AA%3C/text%3E%3C/svg%3E'"/><h4>' + esc(title) + '</h4><div class="card-actions">' + actions + '</div></div>';
}
...
function toggleFav(videoId, btn) {
  ...
  else { const s = songStore[videoId]; if(s){ favs.push(s); btn.classList.add('on'); showToast('Added to favorites ❤️'); } }
  saveFavorites(favs);
}
...
function renderFavorites() {
  ...
  if (!favs.length) { grid.innerHTML = '<div class="empty-state"><div class="empty-icon">❤️</div><h3>No favorites yet</h3><p>Search for songs and tap ❤ to save them here.</p></div>'; return; }
...
function renderPlaylists() {
  ...
  if (!pls.length) { container.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><h3>No playlists yet</h3><p>Create your first playlist above.</p></div>'; return; }
  container.innerHTML = pls.map(pl => '<div class="playlist-item" onclick="openPlaylist(' + pl.id + ')"><div class="playlist-item-left"><div class="playlist-icon">📋</div><div><div class="playlist-item-name">' + esc(pl.name) + '</div><div class="playlist-item-count">' + pl.songs.length + ' song' + (pl.songs.length!==1?'s':'') + '</div></div></div><div class="playlist-item-actions"><button class="btn-icon" onclick="event.stopPropagation();deletePlaylist(' + pl.id + ')">Delete</button></div></div>').join('');
}
...
function openPlaylist(id) {
  ...
  if (!pl.songs.length) { grid.innerHTML = '<div class="empty-state"><div class="empty-icon">🎵</div><h3>Empty playlist</h3><p>Search for songs and tap "+ List" to add them here.</p></div>'; return; }
...
function openPlaylistModal(videoId) {
  ...
  if (!pls.length) { list.innerHTML = '<p style="color:var(--text-muted);font-size:13px;padding:8px 0">No playlists yet. Create one in the Playlists tab.</p>'; }
  else { list.innerHTML = pls.map(pl => '<div class="modal-playlist-option" onclick="addSongToPlaylist(' + pl.id + ')"><span>📋</span> ' + esc(pl.name) + '<span style="margin-left:auto;font-size:11px;color:var(--text-dim)">' + pl.songs.length + ' songs</span></div>').join(''); }
...
function addSongToPlaylist(playlistId) {
  ...
  const pl = pls.find(p => p.id === playlistId);
  if (!pl) return;
  if (!pl.songs.some(s => s.videoId === pendingSong.videoId)) { pl.songs.push(pendingSong); savePlaylists(pls); showToast('Added to "' + pl.name + '"'); }
